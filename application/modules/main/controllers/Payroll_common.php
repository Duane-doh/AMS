...
class Payroll_common extends Main_Controller {
	...
	public function get_system_generated_amount($compensation, $payout_summary_id=NULL, $covered_date_from=NULL, $covered_date_to=NULL, 
			$attendance_period_id=NULL, $sys_params=array(), $attendance_count=array(), $payroll_type_vars=array(), $premium_employee=array(),
			$salary_schedules=array(), $gsis_table=array(), $prorated_rates=array())
			...
			switch ($compensation_code) 
			{
				case $sys_params[PARAM_COMPENSATION_BASIC_SALARY]:
					...
					if ($salary_frequency_flag == SALARY_DAILY)
					{
						...
						// ====================== jendaigo : start : include employees with premium compensation ============= //
						$amount = $this->_get_basic_salary_daily($employee_id, $covered_date_from, $covered_date_to, $salary_frequency_flag, 
									$compensation_id, $sys_params[PARAM_COMPENSATION_ID_BASIC_SALARY_ADJUSTMENT], 
									$attendance_count, $payroll_hdr_id, $payroll_type_id, $premium_employee);	
						// ====================== jendaigo : end : include employees with premium compensation ============= //
					}
					else
					{
	...
	// ====================== jendaigo : start : include employees with premium compensation ============= //		
	private function _get_basic_salary_daily($employee_id, $covered_date_from, $covered_date_to, $salary_frequency_flag, 
			$id_basic_salary, $id_basic_salary_adj, $attendance_count, $payroll_hdr_id, $payroll_type_id=0, $premium_employee=array())
	// ====================== jendaigo : end : include employees with premium compensation ============= //
	{
		try
		{
			...
			*/
			...
			// ====================== jendaigo : start : get bs based on we effectivity date ============= //
			$fields					= array('employ_salary_grade', 'employ_salary_step', 'IFNULL(employ_monthly_salary, 0) employ_monthly_salary', 'employ_start_date', 'IFNULL(employ_end_date, current_date) employ_end_date');
			$emp_salary_recs		= $this->common_model->get_general_data($fields, $this->common_model->tbl_employee_work_experiences, $where, TRUE);

			foreach ($emp_salary_recs as $emp_salary_rec):
				if( ($emp_salary_rec['employ_end_date'] >= $covered_date_from) AND ($emp_salary_rec['employ_start_date'] <= $covered_date_to) )
					$basic_monthly_amount = (isset($emp_salary_rec['employ_monthly_salary']) ? $emp_salary_rec['employ_monthly_salary'] : 0.00);
			endforeach; 
			
			$basic_monthly_amount = isset($premium_employee) ? ($basic_monthly_amount + ($basic_monthly_amount * ($premium_employee['rate']/100))) : $basic_monthly_amount;
			
			// ====================== jendaigo : end : get bs based on we effectivity date ============= //

			$month_work_days = $this->compute_working_days_in_month($covered_date_from, $covered_date_to, FALSE, FALSE);
			 
			$daily_rate		= ROUND(($basic_monthly_amount / $month_work_days), $round_to_5);
			...
			$daily_rate = ROUND( ($basic_monthly_amount/$month_work_days), 5);
			...
			// ====================== jendaigo : start : deactivate condition for basic salary adjustments ============= //
			/*
			...
			*/
			// ====================== jendaigo : end : deactivate condition for basic salary adjustments ============= //
		...
		try
		{
			...
			if ($salary_frequency_flag == SALARY_DAILY)
			{
				...
				if ( ! empty($attendance_count[KEY_TARDINESS_HR])) // late hours
				{
					...
					/* jendaigo: deleted to avoid adding 8hrs late to day absent
					if($time_hr >= 8)
					{
				...
				/* jendaigo: deleeted to avoid adding 8hrs late to day absent
				if($ta_hr >= 8)
				{
				...
				switch ($time_record['attendance_status_id']) 
				{
					...
					case ATTENDANCE_STATUS_REGULAR_DAY: // late
						...
						if ($total_undertime_day >= 8.00)
							$total_subs_deduct_count++;
						// ====================== jendaigo : start : delete condition ============= //
