...
class General_payroll_summary_per_office extends Main_Controller {
	...
	public function generate_report_data($params)
	{
		try
		{
			...
			$param_office         = $this->rm->get_reports_data($field, $table, $where, FALSE);
			$data['office_name']  = $param_office['name'];
			...
			$select_fields  	= array();
			$select_fields[] 	= 'A.less_absence_flag, C.office_id, A.compensation_id, D.attendance_period_hdr_id, A.compensation_name, SUM(ifnull(B.amount,0)) as amount,SUM(ifnull(B.orig_amount,0)) as orig_amount,SUM(ifnull(B.less_amount,0)) as less_amount, A.active_flag, A.inherit_parent_id_flag, A.parent_compensation_id';
			$where         		= array();
			...
			$compensation_temp	= $this->rm->get_reports_data($select_fields, $tables, $where, TRUE, array(), array('A.compensation_id,D.attendance_period_hdr_id,C.office_id'));

			$compensation_records = array();

			foreach ($compensation_temp as $key => $value) {
				$compensation_id = $value['compensation_id'];
				$in_array = in_array($value['office_id'], $office_list);
				...
				if($value['attendance_period_hdr_id'] == $params['payroll_period'] AND $in_array)
					$compensation_records[$compensation_id]['amount'] += $value['amount'];
			}
			
			foreach($compensation_records as $key=>$record)
			{
				if(($record['active_flag'] == NO) && ($record['amount']<=0) OR  $record['exclude'] == TRUE)
				{
			...
			$select_fields  = array();
			$select_fields[] = 'C.office_id, A.deduction_id, D.attendance_period_hdr_id, A.deduction_name, SUM(ifnull(B.amount,0)) as amount, A.active_flag, A.parent_deduction_id, A.inherit_parent_id_flag';
			...
			$deduction_temp 	= $this->rm->get_reports_data($select_fields, $tables, $where, TRUE, array(), array('A.deduction_id,D.attendance_period_hdr_id,C.office_id'));
			...
			foreach ($deduction_temp as $key => $value) {
				...
				$in_array = in_array($value['office_id'], $office_list);
					...
					if(EMPTY($deduction_records[$deduction_id]['attendance_period_hdr_id']))
					{
						...
						elseif($in_array) 
							$deduction_records[$value['deduction_id']]['amount']                   = $value['amount'];
							
							$deduction_records[$value['deduction_id']]['deduction_name']           = $value['deduction_name'];
							$deduction_records[$value['deduction_id']]['deduction_id']             = $value['deduction_id'];
							$deduction_records[$value['deduction_id']]['attendance_period_hdr_id'] = $value['attendance_period_hdr_id'];
						continue;
					} 
					
					if($value['attendance_period_hdr_id'] == $params['payroll_period'] AND $in_array)
						$deduction_records[$deduction_id]['amount'] += $value['amount'];
			}
			...
			$where['B.office_id']                = array($office_list,array('IN'));
...
/* End of file General_payroll_summary_per_office.php */
/* Location: ./application/modules/main/controllers/reports/payroll/General_payroll_summary_per_office.php */