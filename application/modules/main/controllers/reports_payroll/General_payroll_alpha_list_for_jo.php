...
class General_payroll_alpha_list_for_jo extends Main_Controller {
	...
	public function generate_report_data($params)
	{
		try
		{
			...
			$header								 = $this->get_header($where);

			// ====================== jendaigo : start : select atm acct based on effectivity date ============= //
			foreach($header as $hdr)
			{
				if($hdr['start_date'])
					$data['header'][$hdr['employee_id']] = $hdr;
				
				if(empty($data['header'][$hdr['employee_id']]))
					$data['header'][$hdr['employee_id']] = $hdr;
			}
			// ====================== jendaigo : end : select atm acct based on effectivity date ============= //
			...
			// ====================== jendaigo : start : get active compensation premium ============= //
			// STORES COMPENSATION SYSTEM PARAM CODE
			$data['prem_code'] 					= $this->rm->get_sysparam_value(PARAM_COMPENSATION_PREMIUM);

			// GET COMPENSATION PREMIUM RATE
			$fields 							= array('rate', 'report_short_code');
			$where								= array();
			$where['report_short_code']			= $data['prem_code']['sys_param_value'];
			$where['active_flag']				= YES;
			$tables								= $this->rm->tbl_param_compensations;
			$comp_prem							= $this->rm->get_reports_data($fields, $tables, $where, TRUE, NULL, NULL);
			$data['comp_prem']['active_rate'] 	= $comp_prem[0]['rate'];
			// ====================== jendaigo : end : get active compensation premium ============= //
			...
			$deduction_groups[]			= $this->get_deduction_groups_codes(SYS_PARAM_TYPE_ALPHALIST_JO_DED3); //jendaigo : include new ded group
			...
			$fields = array('deduction_id', 'deduction_code','deduction_name',  'report_short_code',  'deduction_type_flag'); //jendaigo : include deduction name and type flag
			...
			// ====================== jendaigo : start : include 8% Income Tax ============= //
			$eight_id 				= $this->rm->get_sysparam_value(PARAM_DEDUCTION_ST_BIR_EIGHT_ID);
			$regular_deduction_ids	= array($ewt_id['sys_param_value'], $gmp_id['sys_param_value'], $eight_id['sys_param_value']);
			// ====================== jendaigo : end : include 8% Income Tax ============= //	
			
			sort($regular_deduction_ids);
			
			//GET REGULAR DEDUCTIONS DETAILS
			// $fields = array('deduction_id', 'report_short_code');
			$fields = array('deduction_id', 'deduction_name', 'report_short_code'); //jendaigo : include deduction name
			...
			// ====================== jendaigo : start : initialization to get premium rate ============= //
			$fields_prem						= array('B.employee_id', 'A.rate');
			$where_prem 						= array();
			$where_prem['A.report_short_code']	= $data['prem_code']['sys_param_value'];
			$where_prem['A.active_flag']		= YES;
			// ====================== jendaigo : end : initialization to get premium rate ============= //
			...
			foreach ($data['header'] as $hdr)
			{
				...
				// ====================== jendaigo : start : store premium rate ============= //
				$where_prem['B.employee_id'] 				= $hdr['employee_id'];
				$where_prem['c.attendance_period_hdr_id'] 	= $params['payroll_period'];
				$premium_dtl								= $this->get_premium($fields_prem, $where_prem);
				$data['records'][$hdr['employee_id']]['premium'] = $premium_dtl['rate'];
				
				$data['comp_prem']['generated_rate'] 		= ($data['records'][$hdr['employee_id']]['premium'] != 0.00 ? $data['records'][$hdr['employee_id']]['premium'] : $data['comp_prem']['generated_rate']);
				// ====================== jendaigo : end : store premium rate ============= //
			}
			...
			foreach($data['records'] as $key=>$record)
			{
				...
				$data['rates'][$key] = $this->compute_rates($key, $record['daily_rate'], $data['records'][$key]['premium']); //jendaigo : include premium rate in the condition

				// STORES DEDUCTIONS IN GROUP AMOUNTS PER ID
				foreach ($data['deduction_groups'] as $group_key => $ded_group)
				{
					// foreach ($ded_group as $ded)
					foreach ($ded_group as $ded_group => $ded)
					{
						$data['ded_amt_in_group'][$key][$group_key][$ded['deduction_id']]	= $this->get_deductions($record['hdr_ids'], $ded['deduction_id']);

						// ====================== jendaigo : start : get deduction detail details ============= //
						if($ded['deduction_type_flag'] == DEDUCTION_TYPE_FLAG_SCHEDULED)

							$ded_amt_dtl_sched = '';							
							$ded_amt_dtl_sched = $this->get_deduction_detail_details($record['attendance_period_hdr_id'], $ded['deduction_id'], $key);

							if($ded_amt_dtl_sched)
							$data['ded_amt_dtl_sched'][$ded['deduction_id']][$key] = $ded_amt_dtl_sched;
						// ====================== jendaigo : end : get deduction detail details ============= //	
					}
				}
	...
	public function get_header($where)
	{
		...
		*/
		...
		/* ===================== jendaigo : start : 
				1. include position & sg
				2. modify fullname format
				3. include new table employee_identification_details in var $tables
				4. include employee_identification_details.employee_identification_id in var $group_by
		============= */
		$fields				= array("A.employee_id", "A.position_name", "A.salary_grade", "D.agency_employee_id", "CONCAT(D.last_name, ', ', D.first_name, IF(D.ext_name='', '', CONCAT(' ', D.ext_name)), IF((D.middle_name='NA' OR D.middle_name='N/A' OR D.middle_name='-' OR D.middle_name='/'), '', CONCAT(' ', D.middle_name))) full_name",
									"G.identification_value tin", "H.identification_value atm_no", "I.start_date");
		...
		$tables		 		= array(
				...
				't9'		=> array(
						...
						'alias' 	=> 'I',
						'type'		=> 'LEFT JOIN',
						'condition'	=> 'H.employee_identification_id = I.employee_identification_id AND 
										IF(I.start_date IS NULL, current_date(), I.start_date) <= C.date_to AND 
										IF(I.end_date IS NULL, current_date(), I.end_date) >= C.date_to'
				)
		);
	
		$group_by					= array('A.employee_id', 'I.employee_identification_id');
		/* ===================== jendaigo : end : 
				1. include position & sg
				2. modify fullname format
				3. include new table employee_identification_details in var $tables
	...
	// ====================== jendaigo : start : get employee premium ============= //
	public function get_premium($fields, $where, $order_by = NULL)
	{
		$tables		 		= array(
				'main'      => array(
				...
				't3'        => array(
						...
						'alias'     => 'C',
						'type'      => 'JOIN',
						'condition' => 'IF(b.start_date IS NULL, current_date(), b.start_date) <= C.date_to'
				)
		);
		
		return $this->rm->get_reports_data($fields, $tables, $where, FALSE, $order_by, NULL);
	}
	// ====================== jendaigo : end : get employee premium ============= //
	...
	public function compute_rates($employee_id, $daily_rate)
	{
		...
		//COMPUTE RATE/DAY
		$rates['rate_day']		= round($daily_rate, 5);
		//COMPUTE RATE/HOUR
	...
	public function get_less_amount($attendance_period_hdr_id, $rate)
	{
		...
		if($total_lwop_hours > 0)
		{
			...
			$lwop			= $this->count_days_hours_minutes(0,$total_lwop_hours); //jendaigo : separate computation of lwop and remove $time conversion to days
		}
		if($time > 0)
		{
			// $undertime      = $this->count_days_hours_minutes($time,false);
			// if($undertime['days'] > 0)
			// $lwop['days']   += $undertime['days'];
			// $undertime['days'] = 0;
			$undertime      = $this->count_days_hours_minutes($time); //jendaigo : remove $time conversion to days
		}
		...
		// ====================== jendaigo : start : separate computation of lwop and remove $time conversion to days ============= //
		$time				= $record['tardiness_hr'] + $record['undertime_hr'] + (($record['tardiness_min'] + $record['undertime_min']) / 60);
		$time_arr			= $this->count_days_hours_minutes($time,$total_lwop_hours);
		// ====================== jendaigo : end : separate computation of lwop and remove $time conversion to days ============= //
	...
	public function count_days_hours_minutes($time, $total_lwop_hours = 0) //jendaigo : separate computation of lwop and remove $time conversion to days
	{
		...
		// ====================== jendaigo : start : separate computation of lwop ============= //
		if($total_lwop_hours>=1)
			$days		= floor($total_lwop_hours / 8);
		// ====================== jendaigo : start : separate computation of lwop ============= //
	...
	public function get_compensations($hdr_ids, $com_id, $basic_salary_flag=FALSE)
	{
		...
		$fields		= array('A.report_short_code', 'A.rate', 'SUM(B.amount) amount', 'SUM(B.orig_amount) orig_amount'); //jendaigo : include rate
	...
	// ====================== jendaigo : start : get deduction detail details ============= //
	public function get_deduction_detail_details($period_hdr_id, $deduction_id, $employee_id)
	{

		$fields		= array('b.amount, e.deduction_detail_type_code, c.employee_deduction_detail_detail_id, d.attendance_period_hdr_id');
		$tables					= array(
			'main'	=> array(
			...
			'table2'=> array(
				...
				'alias'		=> 'c',
				'type'		=> 'INNER JOIN',
				'condition'	=> 'b.employee_deduction_detail_id = c.employee_deduction_detail_id AND
								b.payment_count >= c.paid_count'
			),
			'table3'=> array(
				'table'		=> $this->rm->tbl_attendance_period_hdr,
				'alias'		=> 'd',
				'type'		=> 'INNER JOIN',
				'condition'	=> 'IF(c.start_date IS NULL, current_date(), c.start_date) <= d.date_to',
			),
			'table4'=> array(
				'table'		=> $this->rm->tbl_param_deduction_detail_types,
				'alias'		=> 'e',
				'type'		=> 'LEFT JOIN',
				'condition'	=> 'e.deduction_detail_type_id = c.deduction_detail_type_id',
			),
			'table5'=> array(
				'table'		=> $this->rm->tbl_employee_deduction_paid_count_details,
				'alias'		=> 'f',
				'type'		=> 'INNER JOIN',
				'condition'	=> 'f.employee_deduction_detail_detail_id = c.employee_deduction_detail_detail_id AND
								f.attendance_period_hdr_id = d.attendance_period_hdr_id',
			)
		);
		...
		$where['d.attendance_period_hdr_id']	= $period_hdr_id;

		return $this->rm->get_reports_data($fields, $tables, $where, TRUE);
