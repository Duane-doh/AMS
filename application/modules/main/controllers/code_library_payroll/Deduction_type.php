...
class Deduction_type extends Main_Controller {
	private $module = MODULE_PAYROLL_CL_DEDUCTION_TYPE;
	...
	public function initialize($action_id = NULL, $id = NULL, $salt = NULL, $token = NULL)
	{
		...
		try
		{
			...
			$resources['datatable'][]	= array('table_id' => 'deduction_type_table', 'path' => 'main/code_library_payroll/' . __CLASS__ .'/get_deduction_type_list', 'advanced_filter' => TRUE);
			$resources['load_modal'] 	= array(
				...
				'modal_deduction_type_payroll'	=> array(
						'controller'		=> 'code_library_payroll/deduction_type_payroll',
						'module'			=> PROJECT_MAIN,
						'method'			=> 'modal_deduction_type_payroll',
						'multiple'			=> true,
						'height'			=> '550px',
						'size'				=> 'sm',
	...
	public function get_deduction_type_list()
	{
		...
		try
		{
			...
			//PERMISSIONS
			$permission_view 			= $this->permission->check_permission(MODULE_PAYROLL_CL_DEDUCTION_TYPE, ACTION_VIEW);
			$permission_edit 			= $this->permission->check_permission(MODULE_PAYROLL_CL_DEDUCTION_TYPE, ACTION_EDIT);
			$permission_assign 			= $this->permission->check_permission(MODULE_PAYROLL_CL_DEDUCTION_TYPE, ACTION_ASSIGN);
			$permission_delete 			= $this->permission->check_permission(MODULE_PAYROLL_CL_DEDUCTION_TYPE, ACTION_DELETE);
			...
			foreach ($deduction_types as $aRow):
				...
				$action .= "<a href='#!' class='apply tooltipped md-trigger' data-modal='modal_deduction_type_payroll' data-tooltip='Assign Payroll Payout' data-position='bottom' data-delay='50' onclick=\"modal_deduction_type_payroll_init('".$assign_action."')\"></a>";
		...
		try
		{
			...
			if(!EMPTY($id))
			{
				...
				$resources['single']		= array(
					...
					'parent_deduction' 		=> $data['deduction_info']['parent_deduction_id'],
					'inherit_parent_id_flag' => $data['deduction_info']['inherit_parent_id_flag']

				);
				//EDIT
				$other_deduct_details 				= $this->cl->get_other_deduction_details($deduction_info['deduction_id']);
				$other_deduction_details			= array();
				foreach($other_deduct_details as $other_deduct_detail) {
					$other_deduct_det_id								= $this->hash($other_deduct_detail['other_deduction_detail_id']);
					$other_deduct_detail['other_deduction_detail_id']	= $other_deduct_det_id;
					if(! EMPTY($other_deduct_details['employee_other_ded_dtl_id']) || ISSET($other_deduct_detail['employee_other_ded_dtl_id']))
					{
						$employee_othert_ded_dtl_id							= $this->hash($other_deduct_detail['employee_other_ded_dtl_id']);
						$other_deduct_detail['employee_other_ded_dtl_id'] 	= $employee_othert_ded_dtl_id;
					}
					$other_deduction_details[] 							= $other_deduct_detail;
				}
				$data ['other_deduction_details'] 	= $other_deduction_details;
			}
	...
	public function process()
	{
		try
		{
			...
			$params['employ_type_flag'] = 		ISSET($params['employ_type_flag'])? $params['employ_type_flag'] : PAYROLL_TYPE_FLAG_ALL;
			...
			$fields['parent_deduction_id'] 		= !EMPTY($valid_data['parent_deduction']) ? $valid_data['parent_deduction'] : NULL;
			...
			$audit_fields['parent_deduction_id'] 	= !EMPTY($valid_data['parent_deduction']) ? $valid_data['parent_deduction'] : ' ';
			...
			else
			{
				...
				// GET OTHER DEDUCTION DETAILS
// 				$where			= array();
// 				$key 			= $this->get_hash_key('deduction_id');
// 				$where[$key]	= $params['id'];
// 				$table_other_d_det = $this->cl->tbl_param_other_deduction_details;
// 				$other_deduction_details = $this->cl->get_code_library_data(array("*"), $table_other_d_det, $where, TRUE);
				
				$other_deduct_details 				= $this->cl->get_other_deduction_details($deduction_info['deduction_id']);
// 				echo "<pre>";
// 				print_r($params);
// 				die();
				foreach($other_deduct_details as $other_ded_dtl) {
					// DELETE OTHER DEDUCTION DETAILS
					if(EMPTY($other_ded_dtl['employee_other_ded_dtl_id']) && ! ISSET($other_ded_dtl['employee_other_ded_dtl_id'])) {
						$where			= array();
						$key 			= 'other_deduction_detail_id';
						$where[$key]	= $other_ded_dtl['other_deduction_detail_id'];
				...
				for ($x = 0; $x<count($valid_data['other_detail_name']); $x++ ) {
					...
					$where			= array();
					$key 			= $this->get_hash_key('other_deduction_detail_id');
					$where[$key]	= $params['other_deduction_detail_id'][$x];
					
					$table_other_d_det = $this->cl->tbl_param_other_deduction_details;
					if( EMPTY($params['employee_other_ded_dtl_id'][$x]))
					{
						// DELETE OTHER DEDUCTION DETAIL
						
// 						$this->cl->delete_code_library($table_other_d_det, $where);
						
						// INSERT OTHER DEDUCTION DETAIL
						
						$this->cl->insert_code_library($table_other_d_det, $fields);
					}
					else
					{
						//UPDATE OTHER DEDUCTION DETAIL
						$this->cl->update_code_library($table_other_d_det, $fields, $where);
					}
				}
	...
	public function delete()
	{
		...
		$info 					= array(
			...
			"path"				=> PROJECT_MAIN . '/code_library_payroll/deduction_type/get_deduction_type_list/',
	...
	private function _validate_deduction_type_input($params) 
	{
		try {
			...
			$validation ['parent_deduction'] 	= array (
					'data_type' 				=> 'string',
					'name' 						=> 'Parent Deduction'
			);
			if($params['has_ded_mon'] == 1)
			{
	...
	public function get_months_limit()
	{
		try
		{
			...
			if(!EMPTY($limit))
			{
				if($params['frequency_id'] == FREQUENCY_ANNUAL)
				{
					$months = array(
							1  =>'January',
							2  =>'February',
...
/* Location: ./application/modules/main/controllers/code_library_payroll/Deduction_type.php */