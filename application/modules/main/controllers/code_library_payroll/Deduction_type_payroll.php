...
class Deduction_type_payroll extends Main_Controller {
	...
	public function modal_deduction_type_payroll($action = NULL, $id = NULL, $salt = NULL, $token = NULL)
	{
		try
		{
			$resources 						= array();
			...
			if(!EMPTY($id))
			{
				$data['deduction']	= $this->get_deduction_payroll_details($id);	
				
				if(!EMPTY($data['deduction'][0]['payroll_deduction_id']))
				{
		...
		$this->load->view('code_library/modals/modal_deduction_type_payroll', $data);
	...
	public function process()
	{
		try
		{
			...
			// SERVER VALIDATION
			$valid	= $this->_validate_data_deduction_type_payroll($params, $payroll_types);
			if(!$valid)
			{
			...
			$table 			= $this->cl->tbl_param_payroll_deductions;
			$where			= array();
			$key 			= $this->get_hash_key('deduction_id');
			$where[$key]	= $params['id'];
			$previous  		= $this->cl->get_code_library_data(array("*"), $table, $where, TRUE);

			// START:: DELETE PARAM PAYROLL DEDUCTION
			$this->cl->delete_code_library($table, $where);
			// END:: DELETE FROM param_payroll_deductions FIRST THEN INSERT

			$result			  		= $this->cl->get_code_library_data(array('deduction_id'), $this->cl->tbl_param_deductions, $where, FALSE);
			$fields 				= array();

			$fields['deduction_id']	= $result['deduction_id'];
			...
			$audit_table[]	= $this->cl->tbl_param_payroll_deductions;
	...
	private function _validate_data_deduction_type_payroll($params, $payroll_types)
	{
		$valid = false;
	...
	public function get_deduction_payroll_details($id)
	{
		$fields			= array('A.deduction_name', 'B.payroll_deduction_id', 'B.payroll_type_id', 'GROUP_CONCAT(DISTINCT(B.payout_date_num)) payout_date_num', 'B.deduction_id');
		$tables 		= array(
				'main'		=> array(
...
/* End of file Deduction_type_payroll.php */
/* Location: ./application/modules/main/controllers/code_library_payroll/Deduction_type_payroll.php */