...
class Monthly_report_on_accession extends Main_Controller {
	...
	public function generate_report_data($param = NULL, $office = NULL, $date = NULL, $prepared_by = NULL, $reviewed_by = NULL)
	{
		try
		{
			...
			$tables 		= array(
				...
				'alias'     => 'G',
				'type'      => 'LEFT JOIN',
				'condition' => 'A.employ_personnel_movement_id = G.personnel_movement_id'
			 	)
			);
			...
			$select_fields[] = (USE_ADMIN_OFFICE == YES) ? 'A.admin_office_name AS office' : 'A.employ_office_name AS office';
			$select_fields[] = 'A.employ_position_name position_name';
			$select_fields[] = 'D.employment_status_name';
			$select_fields[] = 'DATE_FORMAT(A.employ_start_date, "%Y/%m/%d") employ_start_date';
			$select_fields[] = 'A.employ_salary_grade';
			$select_fields[] = 'G.personnel_movement_name';
			...
			$where                        																  		= array();			
			$where['A.employ_personnel_movement_id']  													 		= array($value, array("IN"));
			$where['A.employ_start_date']                     													= array($value = array($date_from,$date_to), array("BETWEEN"));
...
/* End of file Monthly_report_on_accession.php */
/* Location: ./application/modules/main/controllers/reports/hr/Monthly_report_on_accession.php */