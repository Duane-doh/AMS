...
class Service_record_model extends Main_Model {
...
EOS;
	...
	public function get_employee_service_record($aColumns, $bColumns, $params, $id)
	{
		try
		{
			$val 	= array($id);
			...
			/* For Advanced Filters */
			$cColumns 	= array("A-service_start", "A-service_end", "D-employment_type_name", "E-station");
			$fields 	= str_replace(" , ", " ", implode(", ", $aColumns));
			...
			$query = <<<EOS
				...
				LEFT JOIN $this->tbl_param_employment_status		D  ON A.service_status  = D.employment_status_id
				LEFT JOIN $this->tbl_param_offices		 		E  ON A.station 		= E.office_id
				LEFT JOIN $this->tbl_param_plantilla			F  ON A.plantilla_id 	= F.plantilla_id
				LEFT JOIN $this->tbl_param_positions				G  ON F.position_id 	= G.position_id
				LEFT JOIN $this->tbl_param_positions				C  ON A.position 		= C.position_id
...
EOS;
	...
	public function filtered_length_employee_service_record($aColumns, $bColumns, $params)
	{
		try
		{
			$this->get_employee_service_record($aColumns, $bColumns, $params);
...
EOS;
	...
	public function total_length_employee_service_record()
	{
		try
		{
			$where = array();
			...
			$fields = array("COUNT(*) cnt");
			return $this->select_one($fields, $this->tbl_employee_service_record, $where);
		}
		catch (PDOException $e)
		{
			$this->rlog_error($e);
	...
	public function get_employee_service_record_info($id)
	{
		try
		{
			$val 	= array($id);
...
EOS;
	...
	public function get_param_employment_type($id)
	{
		...
		{
			$condition_active_flag 	= "active_flag = 'Y'";
			$fields 				= "employment_status_id, employment_type_name";
			$query = <<<EOS
				SELECT $fields
...
EOS;
	...
	public function get_param_leave_type($id)
	{
		...
		{
			$condition_active_flag 	= "active_flag = 'Y'";
			$fields 				= "leave_type_id, leave_type_name";
...
EOS;
	...
	public function get_param_nature_appointment($id)
	{
		try
		{
			$condition_active_flag 	= "active_flag = 'Y'";
			$fields 				= "nature_appointment_id, nature_appointment_name";

			$query = <<<EOS
				SELECT $fields
				FROM $this->tbl_param_nature_appointment
				where $condition_active_flag
EOS;
	...
	public function get_param_plantilla($aColumns, $bColumns, $params, $table, $where, $multiple = TRUE)
	{
		try
		{
			...
			$query = <<<EOS
				...
				JOIN $this->tbl_param_appointment_status D ON A.appointment_status_id = D.appointment_status_id
