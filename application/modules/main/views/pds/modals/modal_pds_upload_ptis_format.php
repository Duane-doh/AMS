<form id="pds_upload_form">
	...
	<div class="form-float-label">
	   <div class="row m-n field-multi-attachment">
			<div class="col s12">
				<div id="attachment_upload_ptis">Select File</div>
			</div>
		</div>
	</div>
	<div class="md-footer default">
	  <a class="btn btn-success-flat cancel_modal">Cancel</a>
	    <button type="button" class="btn btn-success " id="save_pds_upload_ptis_format" value="<?php echo BTN_SAVE ?>"><?php echo BTN_SAVE ?></button>
	</div>
</form>

<script>
$(function(){
	jQuery(document).off('click', '#save_pds_upload_ptis_format');
	jQuery(document).on('click', '#save_pds_upload_ptis_format', function(e){
  ...
  $('#pds_upload_form').parsley();
   jQuery(document).off('submit', '#pds_upload_form');
	...
	if ( $(this).parsley().isValid() ) {
	  ...
	  button_loader('save_pds_upload_ptis_format', 1);

	  $.post("<?php echo base_url() . PROJECT_MAIN ?>/Pds_upload_ptis_format/process_pds_upload/", data, function(result) {
		if(result.status){
			if(result.proceed_flag == 'Y')
			{
				notification_msg("<?php echo SUCCESS ?>", result.message);
				button_loader("save_pds_upload_ptis_format",0);
				modal_pds_upload_ptis_format.closeModal();
	...
	var uploadObj = $("#attachment_upload_ptis").uploadFile({
		url: $base_url + "upload/",
		fileName: "file",
		allowedTypes:"xlsx",
		acceptFiles:"*",	
