<form id="delete_personnel_form">	
	...
	<div class="row">
		...
		<div class="col s6">
			<div class="row b-n p-t-lg p-l-md"><span class="font-md font-playfair-display font-spacing-1">Personnel Category Filter</span></div>
			 <div class="form-float-label">
				...
				<div class="row m-n">
					<div class="col s12">
					  <div class="input-field">
					  	<label for="remove_personnel_name" class="active">Personnel Name</label>
						<select id="remove_personnel_name" multiple name="personnel_name[]" class="selectize" placeholder="Select Personnel Name">
				...
				<div class="row m-n b-b-n b-r-n b-l-n">
					<div class="col s12 b-n">
				     	<button type="button" class="btn btn-success blue pull-right" id="add_to_remove_list" >Add To List</button>
					</div>
				</div>
			</div>
		</div>
		  	...
		  	<div class="scroll-pane" style="height:370px;">
		  		<div class="row p-md">
		  			<table class="striped table-default">
		  				<thead>
		  					<tr>
			  					<th width="30%">Personnel Number</th>
			  					<th width="60%">Personnel Name</th>
...
$(document).ready(function(){
	...
	$('#office,#position,#salary_grade').on('change', function(e){
		...
		var data = $('#delete_personnel_form').serialize();
	...
	$('#delete_personnel_form').parsley();
	
 	jQuery(document).off('submit', '#delete_personnel_form');
	jQuery(document).on('submit', '#delete_personnel_form', function(e){
	    e.preventDefault();
		...
		if ( $(this).parsley().isValid() ) {
			var data = $('#delete_personnel_form').serialize();
		  	...
		  	var option = {
					url  : $base_url + 'main/compensation/process_remove_personnel',
					data : data,
					success : function(result){
						if(result.status)
						{
							...
							load_datatable('table_leave_type_adjustment', '<?php echo PROJECT_MAIN ?>/compensation/get_leave_type_employee_list',false,0,0,true,post_data);
						}
						else
						{
